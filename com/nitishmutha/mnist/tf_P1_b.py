...
def NN_P1_B():
  ...
  W2 = tf.Variable(tf.truncated_normal([HIDDEN_LAYER_1, OUTPUT_NODES], stddev=0.1))
  ...
  with tf.Session() as sess:
    ...
    if (TRAIN_MODE):
      ...
      #Run test
      saver.restore(sess, 'trained_weights/P1_b/tf_P1_b.ckpt')
      acc, pred = sess.run([accuracy, y_p], feed_dict={x: mnist.test.images, y_: mnist.test.labels})
      print("Test accuracy %f" % acc)


    if (TRAIN_MODE):
      save_path = saver.save(sess, 'trained_weights/P1_b/tf_P1_b.ckpt')
      print('Model weights saved in file: ', save_path)

      plotGraph(errorList, 'train cross entropy cost', 'epoch', 'cost', 'P1(b): Cross entropy cost')
      plotGraph(accuracyList, 'classification accuracy', 'epoch', 'accuracy', 'P1(b): Accuracy per epoch')
